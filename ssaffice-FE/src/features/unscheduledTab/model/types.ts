import type { createUser } from '@/features/announcementTab'
// 유저 타입은 아래 공지, 일정 타입에도 다 들어가 있지만,
// API 응답에서 데이터 계층구조가 변동되어 유저가 공지/일정에 소속되지 않고 독립적으로 오는 경우 사용해 주세요
// USER 타입이 독립적으로 오게 되면 컴포넌트 표기 로직도 일부 수정해야 합니다.

import type { AnnouncementItemDisplay } from '@/features/announcementTab'
import type { TodoItemDisplay } from '@/features/todoTab'

// 미등록 공지 타입은 일정 + 공지가 더해진 형태의 타입입니다.
export type UnscheduledItemDisplay = {
  todo: TodoItemDisplay
  announcement: AnnouncementItemDisplay
  // user: createUser // 유저 정보가 일정/공지에서 독립되어 오는 경우
}

export type UnscheduledListDisplay = Record<string, UnscheduledItemDisplay[]>
// 날짜로 묶은 미등록 공지 items

// 더미 데이터 배열입니다.
// MARK: datas
export const dummyUnschedules: UnscheduledItemDisplay[] = [
  {
    todo: {
      scheduleId: 1,
      title: '프로젝트 미팅',
